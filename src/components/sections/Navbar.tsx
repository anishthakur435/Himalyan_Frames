        ...
        <div className="flex items-center justify-between h-20">
          ...
          <div className="flex md:hidden items-center space-x-3">
            ...
            <Sheet open={isOpen} onOpenChange={setIsOpen}>
              ...
              <SheetContent side="top" className="bg-app-bg border-b border-app-border pt-20 px-4">
                <SheetTitle className="sr-only">Navigation Menu</SheetTitle>
                <SheetDescription className="sr-only">Select a page to navigate to.</SheetDescription>
                <div className="pt-4 pb-6 space-y-3 bg-app-bg transition-colors duration-500">
                  {navItems.map((item) => (
                    <Link
                      key={item.id}
                      id={`mobile-nav-${item.id.replace('/', '')}`}
                      href={item.id}
                      onClick={() => setIsOpen(false)}
